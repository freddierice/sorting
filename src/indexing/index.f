...
#define MAX_READ 10
      ...
      subroutine Sortbycolumn(filename,datasetname,columns,columncount,rowcount,irank,isize,icomm,istatus,iinfo)
         ...
         !temporary variables
         double precision,dimension(:),allocatable :: chunk
         integer,dimension(:),allocatable :: chunkIndex
         integer,dimension(:,:),allocatable :: ord
         integer(HSIZE_T),dimension(:,:),allocatable :: coords
         ...
         real :: rtemp
         integer :: i1,i2,i3,i4
         logical :: isFirstIndex
         character(len=6) :: datasetIn, datasetOut
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         chunks = int(rtemp)
         if( real(chunks) .ne. rtemp ) chunks = chunks + 1

         dim2(1) = MAX_READ
         call h5screate_simple_f(1, dim2, sLargeMemory, herror)
         
         allocate(ord(2,MAX_READ))
         ord(1,:) = 1
         hCount(1) = MAX_READ
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         !allocate memory and write out sequence of numbers
         !each array only allocated/deallocated twice.
         call h5dget_space_f(dSwap_Index1,sSwap_Index1, herror)
         call h5dget_space_f(dData_Data,SData_Data, herror)
         dim1(1) = MAX_READ
         call h5screate_simple_f(1,dim1,sMemory,herror)
         allocate(ord(2,MAX_READ))
         allocate(coords(2,MAX_READ))
         allocate(chunk(MAX_READ))
         ...
         do i=1,chunks
            ...
            if( chunks .eq. i ) then
               ...
               deallocate(coords)
               call h5sclose_f(sMemory,herror)
               currentSize = maximum-minimum-MAX_READ*(chunks-1) + 1
               allocate(ord(2,currentSize))
               allocate(coords(2,currentSize))
               allocate(chunk(currentSize))
               dim1(1) = currentSize
               call h5screate_simple_f(1,dim1,sMemory,herror)
               ord(1,:) = currentColumn
               coords(1,:) = currentColumn
            ...
            call h5sselect_none_f(sSwap_Index1,herror)
            call h5sselect_hyperslab_f(sSwap_Index1, H5S_SELECT_SET_F, hOffset, hCount, herror)
            ...
            !call h5dwrite_f(dData_Data, H5T_NATIVE_DOUBLE, dat, hCount2, herror, mem_space_id=sMemory, file_space_id=sData_Data)
            ord = coords
!           print *,"Kind of: ", int(chunk)
!           print *,"Before: ", ord(2,:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         do i=1,chunks
            if( i .eq. chunks) then
               call enqueue(MAX_READ*(i-1) + minimum, queue)
               call enqueue(maximum, queue)
               exit
            end if
            call enqueue(MAX_READ*(i-1) + minimum, queue)
            call enqueue(MAX_READ*i+minimum-1, queue)
         end do

         !sort the indexes in the queue
         do while( QueueCardinality(queue) .ne. 2 )
            
            !pull indeces from queue
            call dequeue(queue, i1)
            call dequeue(queue, i2)
            call dequeue(queue, i3)
            call dequeue(queue, i4)
            ...
            do while( .true. ) 
               ...
               if(i2 + 1 .ne. i3) then
                  call enqueue(i3, queue)
                  call enqueue(i4, queue)
                  call dequeue(queue, i3)
                  call dequeue(queue, i4)
               else
                  exit
            ...
            call enqueue(i1,queue)
            call enqueue(i4,queue)
            isFirstIndex = not(isFirstIndex)
         end do
